        ...
        <?php if(isset($data['edit'])){ ?>
            <div id="overlayCategory" class="fixed  inset-0 top-0 left-0 bg-black w-full h-full bg-opacity-30 backdrop-blur-sm  z-50 flex items-center justify-center">
                ...
                <form action="<?= URLROOT . 'Categories/edit' ?>" method="POST" id="formCategory" class="w-[550px] bg-white p-5 rounded-lg relative">
                    ...
                    <input value="<?= $data['category']->id_category ?>" type="hidden" id="id" class=" bg-white border text-sm rounded-lg focus:ring-red-500  focus:border-red-500 block w-full p-2.5 " placeholder="Enter Category name" name="id">

                    <p class="mt-2 text-md opacity-0 font-medium text-red-600 bg-red-50 py-2 px-3 rounded-lg dark:text-red-500" id="fieldErr"></p>
                    <div class="py-3">
                        <label for="error" class="block mb-2 text-md font-medium text-secondary">Name Category</label>
                        <input value="<?= $data['category']->name ?>" type="text" id="name" class=" bg-white border text-sm rounded-lg focus:ring-red-500  focus:border-red-500 block w-full p-2.5 " placeholder="Enter Category name" name="name">
                    </div>
                    ...
                    <table id="example" class="display" style="width:100%">
                            ...
                            <?php foreach($data as $category):?>
                                <tr class="border-[1px] border-black bg-gray-200">
                                    <td class="w-1/3 text-left py-3 px-4"><?= $category->id_category ?></td>
                                    <td class="w-1/3 text-left py-3 px-4"><?= $category->name ?></td>
                                    <td class="w-1/3 text-left py-3 px-4"><button type="button"><a href="<?= URLROOT . 'categories/delete/' .  $category->id_category ?>">Delete</a></button><button type="button"><a href="<?= URLROOT . 'categories/get/' .  $category->id_category ?>">Edit</a></button></td>
                                </tr>
                            <?php endforeach;  ?>
                            </tbody>
                        </table>
