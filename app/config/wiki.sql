CREATE DATABASE IF NOT EXISTS wiki;
USE wiki;

CREATE TABLE IF NOT EXISTS Role(
name VARCHAR(50) PRIMARY KEY 

);

CREATE TABLE IF NOT EXISTS User(
id_user INT(50) PRIMARY KEY AUTO_INCREMENT,
full_name VARCHAR(50),
username VARCHAR(50),
email VARCHAR(50),
password VARCHAR(50),
name_role VARCHAR(50),
FOREIGN KEY (name_role) REFERENCES Role(name) ON DELETE CASCADE ON UPDATE CASCADE
);

CREATE TABLE IF NOT EXISTS Category(
id_category INT(50) PRIMARY KEY AUTO_INCREMENT,
name VARCHAR(50)
);

CREATE TABLE IF NOT EXISTS Tag(
id_tag INT(50) PRIMARY KEY AUTO_INCREMENT,
name VARCHAR(50)
);

CREATE TABLE IF NOT EXISTS Wiki(
id_wiki INT(50) PRIMARY KEY AUTO_INCREMENT,
title VARCHAR(50),
content VARCHAR(255),
date_create VARCHAR(50),
date_modified VARCHAR(50),
id_user INT(50),
id_category INT(50),
FOREIGN KEY (id_user) REFERENCES User(id_user) ON DELETE CASCADE ON UPDATE CASCADE,
FOREIGN KEY (id_category) REFERENCES Category(id_category) ON DELETE CASCADE ON UPDATE CASCADE
);

CREATE TABLE IF NOT EXISTS TagOfWiki(
id_tag INT(50) ,
id_wiki INT(50),
FOREIGN KEY (id_tag) REFERENCES Tag(id_tag) ON DELETE CASCADE ON UPDATE CASCADE,
FOREIGN KEY (id_wiki) REFERENCES Wiki(id_wiki) ON DELETE CASCADE ON UPDATE CASCADE
);
